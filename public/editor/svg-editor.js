	...
	if(!window.svgEditor) window.svgEditor = function($) {
		...
		curConfig = {
		  ...
		  initStroke: {width: 1.5, color: '000', opacity: 1},
			...
			baseUnit: 'px',
			snappingStep: 10,
			showRulers: (svgedit.browser.isTouch()) ? false : false,
		...
		Editor.init = function() {
  		...
  		$("#base_unit")
			...
			setTimeout(function(){$('#workarea').scrollTop(0)});
      ...
      $('#tool_fill').on("touchstart", function(){
	    ...
	    $('#stroke_width') .dragInput({ min: 0,    max: 99,    step:  1,  callback: changeStrokeWidth,   cursor: true, smallStep: 0.1, start: 1.5          });
          ...
          $("#workarea").removeAttr("style");
					$('#main_menu').hide();
					var file = null;
					var element = e.dataTransfer.getData('svg/xml')
					//it's dragged svg
					if (element.length) {
			  		svgCanvas.importSvgString(element);
		  			svgCanvas.ungroupSelectedElement()
		  			//svgCanvas.ungroupSelectedElement()
		  			svgCanvas.groupSelectedElements()
		  			root_sctm = svgcontent.getScreenCTM().inverse();
						isBotchedZoom = svgedit.browser.isGecko();
						var pt = svgedit.math.transformPoint( e.pageX, e.pageY, root_sctm ),
						current_zoom = svgCanvas.getZoom();
						mouse_x = pt.x * (isBotchedZoom ? 1 : current_zoom),
						mouse_y = pt.y * (isBotchedZoom ? 1 : current_zoom);
						var inserted = svgCanvas.getSelectedElems()[0];
						var bbox = inserted.getBBox()
		  			svgCanvas.moveSelectedElements(mouse_x-bbox.width/2-100, mouse_y-bbox.height/2-100, false);
		  			return;
					}
