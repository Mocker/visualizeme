...
svgEditor.addExtension("shapes", function() {
	...
	function loadIcons() {
		...
		// Show lib ones
		$('#newshapelib').append(cur_lib.buttons);
	}
	...
	function makeButtons(cat, shapes) {
		...
		for(var id in data) {
			...
			var icon_btn = icon.wrap('<div class="module">').parent().attr({
				id: mode_id + '_' + id,
				title: id
			});
	...
	return {
		...
		callback: function() {
			...
			var btn_div = $('<div id="module">');
			$('#newshapelib > *').wrapAll(btn_div);
			...
			$('#newshapelib').mouseup(function(evt) {
